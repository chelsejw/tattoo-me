...
module.exports = (dbPoolInstance) => {
    ...
    let clearTattooHashtags = (tattooId, callback) => {

        let query = `DELETE FROM tattoos_hashtags WHERE tattoo_id = ${tattooId}`;
    ...
    const deleteTattooById = (tattooId, callback)=>{

        let query = `DELETE FROM tattoos WHERE tattoo_id = ${tattooId}`;
        dbPoolInstance.query(query, (err, result) => {
          if (err) {
            return callback(err, null);
          } else if (result.rows.length < 1) {
    ...
    return {
        ...
        clearTattooHashtags: clearTattooHashtags,
        deleteTattooById: deleteTattooById
    };
};